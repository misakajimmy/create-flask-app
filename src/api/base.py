...
def get_register_code():
    ...
    """
    phone = str(g.request_data.get('phone', '')).strip()

    if not is_mobile(phone):
        current_app.logger.debug('手机号参数错误: phone:%s', phone)
        return api_return('PARAM_MOBILE_ERROR', '手机号参数错误')

    user = UserInfo.objects(phone=phone).first()
    if user:
        current_app.logger.debug('手机号已存在: phone:%s', phone)
        return api_return('DATA_EXIST', '手机号已存在')
    ...
    count = VerifySMS.objects(app_name=app_name, phone=phone, business=sms_business) \
        .filter(create_time__gt=last_time).count()
    if count >= code_limit:
        current_app.logger.debug('请求次数超出限制: phone:%s, count:%s, code_limit:%s', phone, count, code_limit)
        return api_return('REQUEST_NUMBER_LIMIT', '请求次数超出限制')

    # code_interval = int(current_app.config.get('SMS_CODE_INTERVAL_TIME', 60))
    # last = VerifySMS.query.filter_by(app_name=app_name, phone=phone, business=sms_business)\
    #     .order_by(VerifySMS.id.desc()).first()
    # if last and (datetime.now() - last.create_time).total_seconds() < code_interval:
    #     current_app.logger.debug('请求频率超出限制: phone:%s', phone)
    #     return api_return('REQUEST_INTERVAL_LIMIT', '请求频率超出限制')

    # 发送短信验证码
    code = create_random_num(6)
    reset_code = VerifySMS(app_name=app_name, phone=phone, business=sms_business, code=code)
    ...
    try:
        reset_code.save()
        current_app.logger.info('获取成功: phone:%s', phone)
        return api_return('OK', '获取成功', res_data)
    except Exception as e:
        current_app.logger.error('获取失败: phone:%s, %s', phone, e)
...
def register():
    ...
    user = UserInfo.objects(account=account).first()
    if user:
    ...
    app_name = current_app.config.get('APP_NAME', '')
    sms_business = 'register'
    code = VerifySMS.objects(
        app_name=app_name,
        phone=phone,
        business=sms_business,
        code=code,
        status=0
    ).first()
    if code is None:
    ...
    user = UserInfo(account=account, phone=phone)
    ...
    # 格式化返回值
    login_info = {
        'user_id': str(user.id), 'account': user.account, 'nickname': user.nickname, 'phone': user.phone,
    ...
    return api_return('OK', '注册成功', data=login_info)
...
def login():
    ...
    if not account or not is_password(password):
        current_app.logger.debug('密码参数错误: account:%s, password:%s', account, password)
        return api_return('PARAM_FORMAT_ERROR', '账号密码参数错误')
    ...
    # 格式化返回值
    login_info = {
        'user_id': str(user.id), 'account': user.account, 'phone': user.phone, 'nickname': user.nickname,
        'avatar_url': user.avatar_url,
        'sex': user.sex, 'birthday': user.birthday if user.birthday else '',
        'age': user.age, 'token': token
    }
    ...
    try:
        current_app.logger.info('登录成功: account:%s, app_channel:%s, app_version:%s, os_type:%s, os_version:%s', account,
                                user.app_channel, user.app_version, user.os_type, user.os_version)
    except Exception as e:
...
def get_user_info():
    ...
    data = {'user_id': str(g.user.id), 'account': g.user.account, 'nickname': g.user.nickname,
    ...
    try:
        ...
        current_app.logger.info('查询成功: account:%s, app_channel:%s, app_version:%s, os_type:%s, os_version:%s',
                                g.user.account, g.user.app_channel, g.user.app_version, g.user.os_type,
                                g.user.os_version)
    except Exception as e:
...
def get_reset_password_code():
    ...
    user = UserInfo.objects(phone=phone).first()
    if user is None:
        current_app.logger.debug('账号不存在: phone:%s', phone)
        return api_return('USER_NOT_FOUND', '账号不存在')
    if user.status != 1:
        current_app.logger.debug('账号已禁用: phone:%s, status:%s', phone, user.status)
        return api_return('USER_INVALID', '账号已禁用')
    ...
    code_interval = int(current_app.config.get('SMS_CODE_INTERVAL_TIME', 60))
    last = VerifySMS.objects(app_name=app_name, phone=phone, business=sms_business) \
        .order_by('-name').first()
    if last and (datetime.now() - last.create_time).total_seconds() < code_interval:
        current_app.logger.debug('请求频率超出限制: phone:%s', phone)
        return api_return('REQUEST_INTERVAL_LIMIT', '请求频率超出限制')
    ...
    if current_app.config.get('ENV') == 'development':  # 测服直接把验证码从接口返回
        ...
        business_id = create_uuid_str()
        sms_res = send_sms_by_alisms(app_name, phone, 'SMS_118315037', {'code': code}, business_id)
        if not sms_res:
            current_app.logger.error('短信发送失败: phone:%s, code:%s', phone, code)
            return api_return('SMS_SEND_FAILED', '短信发送失败，请稍后再试！')
        reset_code.business_id = business_id
        res_data = None
...
def reset_password():
    ...
    if not is_mobile(phone):
        current_app.logger.debug('手机号参数错误: account:%s', phone)
        return api_return('PARAM_MOBILE_ERROR', '手机号参数错误')
    if not is_password(password):
        current_app.logger.debug('密码参数错误: account:%s, password:%s', account, password)
        return api_return('PARAM_FORMAT_ERROR', '密码参数错误')
    ...
    sms_business = 'reset_password'
    code = VerifySMS.objects(
        app_name=app_name, phone=phone, business=sms_business, code=code,
        status=0).first()
    if code is None:
        current_app.logger.debug('验证码无效: phone:%s, code:%s', phone, code)
        return api_return('PARAM_CODE_ERROR', '验证码无效')
    valid_time = current_app.config.get('CODE_VALID_TIME', 10 * 60)
    if (datetime.now() - code.create_time).total_seconds() > valid_time:
        current_app.logger.debug('验证码已过期: phone:%s, code:%s', phone, code)
        return api_return('PARAM_CODE_INVALID', '验证码已过期')
    ...
    # 格式化返回值
    login_info = {
        'user_id': str(user.id), 'account': user.account, 'nickname': user.nickname, 'avatar_url': user.avatar_url,
        'sex': user.sex, 'birthday': user.birthday if user.birthday else '',
        'age': user.age, 'token': token
    }
...
def get_change_account_code():
    ...
    #group  基础
    #name   获取修改手机号的验证码
    #desc   获取修改手机号的验证码，验证码通过短信下发
    #priv   need_login
    #param  new_phone   <str>    新手机号
    ...
    """
    new_phone = str(g.request_data.get('new_phone', '')).strip()
    password = str(g.request_data.get('password', '')).strip()

    if not is_mobile(new_phone):
        current_app.logger.debug('手机号参数错误: account:%s, new_account:%s', g.user.account, new_phone)
        return api_return('PARAM_MOBILE_ERROR', '手机号参数错误')
    ...
    # 验证新手机号是否已绑定
    new_user = UserInfo.objects(phone=new_phone).first()
    if new_user:
        current_app.logger.debug('账号已被使用: account:%s, new_account:%s', g.user.account, new_phone)
    ...
    count = VerifySMS.objects(app_name=app_name, phone=new_phone, business=sms_business) \
        .filter(create_time__gt=last_time).count()
    if count >= code_limit:
        current_app.logger.debug(
            '请求次数超出限制: account:%s, new_phone:%s, count:%s, code_limit:%s', g.user.account,
            new_phone, count, code_limit)
        return api_return('REQUEST_NUMBER_LIMIT', '请求次数超出限制')

    code_interval = int(current_app.config.get('SMS_CODE_INTERVAL_TIME', 60))
    last = VerifySMS.objects(app_name=app_name, phone=new_phone, business=sms_business) \
        .order_by('-id').first()
    if last and (datetime.now() - last.create_time).total_seconds() < code_interval:
        current_app.logger.debug('请求频率超出限制: account:%s, new_phone:%s', g.user.account, new_phone)
        return api_return('REQUEST_INTERVAL_LIMIT', '请求频率超出限制')

    # 发送短信验证码
    code = create_random_num(6)
    reset_code = VerifySMS(app_name=app_name, phone=new_phone, business=sms_business, code=code)
    if current_app.config.get('ENV') == 'development':  # 测服直接把验证码从接口返回
        res_data = {'code': code}
    else:
        business_id = create_uuid_str()
        sms_res = send_sms_by_alisms(app_name, new_phone, 'SMS_118315037', {'code': code}, business_id)
        if not sms_res:
            current_app.logger.error('短信发送失败: account:%s, new_phone:%s, code:%s', g.user.account, new_phone, code)
            return api_return('SMS_SEND_FAILED', '短信发送失败，请稍后再试！')
        reset_code.business_id = business_id
        res_data = None
    ...
    try:
        reset_code.save()
        current_app.logger.info('获取成功: account:%s, new_phone:%s', g.user.account, new_phone)
        return api_return('OK', '获取成功', res_data)
    except Exception as e:
        current_app.logger.error('获取失败: account:%s, new_phone:%s, %s', g.user.account, new_phone, e)
        return api_return('FAILED', '获取失败')
...
def change_account():
    ...
    if not is_mobile(new_phone):
        current_app.logger.debug('手机号参数错误: account:%s, new_phone:%s', g.user.account, new_phone)
        return api_return('PARAM_MOBILE_ERROR', '手机号参数错误')
    if not is_sms_code(code):
        current_app.logger.debug('验证码参数错误: account:%s, new_phone:%s, code:%s', g.user.account, new_phone, code)
        return api_return('PARAM_FORMAT_ERROR', '验证码参数错误')

    # 验证新账号是否已注册
    new_user = UserInfo.objects(phone=new_phone).first()
    if new_user:
        current_app.logger.debug('账号已被使用: account:%s, new_phone:%s', g.user.account, new_phone)
        return api_return('PARAM_ERROR', '账号已被使用')

    app_name = current_app.config.get('APP_NAME', '')
    sms_business = 'change_account'
    code = VerifySMS.objects(
        app_name=app_name, phone=new_phone, business=sms_business, code=code,
        status=0
    ).first()
    if code is None:
        current_app.logger.debug('验证码无效: account:%s, new_phone:%s, code:%s', g.user.account, new_phone, code)
        return api_return('PARAM_CODE_ERROR', '验证码无效')
    valid_time = current_app.config.get('CODE_VALID_TIME', 10 * 60)
    if (datetime.now() - code.create_time).total_seconds() > valid_time:
        current_app.logger.debug('验证码已过期: account:%s, new_phone:%s, code:%s', g.user.account, new_phone, code)
        return api_return('PARAM_CODE_INVALID', '验证码已过期')

    code.status = 1  # 修改验证码状态为已使用
    g.user.phone = new_phone  # 修改手机号
    try:
        code.save()
        g.user.save()
        current_app.logger.info('修改成功: account:%s, new_phone:%s', g.user.account, new_phone)
        return api_return('OK', '修改成功')
    except Exception as e:
        current_app.logger.error('修改失败: account:%s, new_phone:%s, %s', g.user.account, new_phone, e)
        return api_return('FAILED', '修改失败')
